  ...
  try {
    ...
    micProc.onaudioprocess = (e) => {
      ...
      while (micAccumulated.length >= SAMPLES_PER_CHUNK) {
        ...
        // Forward audio: if central manager path available use IPC, otherwise fallback to local WS
        try {
          if ((window as any).evia?.audio?.sendMicFrame) {
            (window as any).evia.audio.sendMicFrame(
          ...
          console.error("[mic] forward failed", err);
        }
