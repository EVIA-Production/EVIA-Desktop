  ...
  try {
    ...
    const onMessage = (msg: string) => {
      try {
        ...
        log(
          `[system msg:${data.type}] ${JSON.stringify(data).substring(0, 100)}${data.type === "transcript_segment" ? "..." : ""}`
        );
    ...
    const onMicMessage = (msg: string) => {
      try {
        const data = JSON.parse(msg);
        log(
          `[mic msg:${data.type}] ${JSON.stringify(data).substring(0, 100)}${data.type === "transcript_segment" ? "..." : ""}`
        );
        ...
        window.evia.systemAudio.onData(async (data: string) => {
          try {
            ...
            if (pcm16 && pcm16.length > 0) {
              ...
              log(
                `[system] Processed chunk RMS=${rms.toFixed(4)} sampleCount=${pcm16.length}`
              );
  ...
  try {
    ...
    micProc.onaudioprocess = (e) => {
      ...
      while (micAccumulated.length >= SAMPLES_PER_CHUNK) {
        ...
        log(
          `[mic] Processed chunk RMS=${rms.toFixed(4)} sampleCount=${pcm16.length}`
        );
...
function exportWav(
  ...
  log(
    `[Export] Saved ${name}.wav (${(dataSize / 2) | 0} samples @ ${sampleRate}Hz)`
  );
}
