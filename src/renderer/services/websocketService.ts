...
function getBackendHttpBase(): string {
  const fromWin =
    (window as any).EVIA_BACKEND_URL || (window as any).API_BASE_URL;
  if (typeof fromWin === "string" && fromWin.trim())
    return String(fromWin).replace(/\/$/, "");
...
export class ChatWebSocket {
  ...
  async connect(attempt: number = 1): Promise<void> {
    try {
      ...
      const wsUrl = `${wsBase}/ws/transcribe?chat_id=${encodeURIComponent(
        chatId
      )}&token=${encodeURIComponent(token)}${sourceParam}&sample_rate=24000`;
      return new Promise((resolve, reject) => {
        ...
        this.ws.onmessage = (event) => {
          try {
            ...
            console.error(
              "[WS Debug] Failed to parse/handle message:",
              e,
              "Raw:",
              event.data
            );
  ...
  sendBinaryData(data: ArrayBuffer) {
    ...
    if (hasAudio && !this.audioDetected) {
      console.log(
        `[Audio Logger] Audio detected - Level: ${audioLevel.toFixed(4)}`
      );
      this.audioDetected = true;
    } else if (!hasAudio && this.audioDetected) {
      console.log(
        `[Audio Logger] Audio ended - Level dropped to ${audioLevel.toFixed(4)}`
      );
    ...
    this.ws.send(data);
    console.log(
      `[Audio Logger] Audio data sent - Size: ${
        data.byteLength
      } bytes, Level: ${audioLevel.toFixed(4)}`
    );
  }
  ...
  onMessage(handler: (message: WebSocketMessage) => void) {
    console.log(
      "[WS Debug] Registering message handler for chatId:",
      this.chatId,
      "Total handlers after:",
      this.messageHandlers.length + 1
    );
    this.messageHandlers.push(handler);
    return () => {
      console.log(
        "[WS Debug] Unregistering message handler for chatId:",
        this.chatId,
        "Total handlers before:",
        this.messageHandlers.length
      );
