...
class ProcessManager {
  ...
  async startSystemAudioHelper() {
    if (os.platform() !== 'darwin') {
      return { ok: false, error: 'System audio helper is only available on macOS' };
    }
  ...
  registerSystemAudioHandlers(stdoutHandler, stderrHandler) {
    ...
    this.processes.systemAudio.stdout.on('data', (chunk) => {
      ...
      let newlineIndex;
      while ((newlineIndex = this.stdoutBuffers.systemAudio.indexOf('\n')) !== -1) {
        const line = this.stdoutBuffers.systemAudio.slice(0, newlineIndex).trim();
        this.stdoutBuffers.systemAudio = this.stdoutBuffers.systemAudio.slice(newlineIndex + 1);
