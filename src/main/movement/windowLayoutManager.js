...
class WindowLayoutManager {
  ...
  calculateSettingsWindowPosition() {
    ...
    const clampedX = Math.max(
      workAreaX + 10,
      Math.min(workAreaX + screenWidth - settingsBounds.width - 10, x)
    );
    const clampedY = Math.max(
      workAreaY + 10,
      Math.min(workAreaY + screenHeight - settingsBounds.height - 10, y)
    );
  ...
  calculateHeaderResize(header, { width, height }) {
    ...
    const clampedX = Math.max(
      workAreaX,
      Math.min(workAreaX + workAreaWidth - width, newX)
    );
  ...
  calculateClampedPosition(header, { x: newX, y: newY }) {
    ...
    const clampedX = Math.max(
      workAreaX,
      Math.min(newX, workAreaX + width - headerBounds.width)
    );
    const clampedY = Math.max(
      workAreaY,
      Math.min(newY, workAreaY + height - headerBounds.height)
    );
  ...
  calculateWindowHeightAdjustment(senderWindow, targetHeight) {
    ...
    console.log(
      `[Layout Debug] calculateWindowHeightAdjustment: targetHeight=${targetHeight}`
    );
  ...
  calculateFeatureWindowLayout(visibility, headerBoundsOverride = null) {
    const header = this.windowPool.get("header");
    const headerBounds =
      headerBoundsOverride || (header ? header.getBounds() : null);
    ...
    const headerCenterXRel =
      headerBounds.x - workAreaX + headerBounds.width / 2;
    ...
    try {
      console.log(
        `[Layout Debug] Strategy: ${strategy.name} (primary=${strategy.primary}, secondary=${strategy.secondary})`
      );
    } catch {}
    ...
    if (askVis) {
      console.log(
        `[Layout Debug] Ask Window Bounds: height=${askB.height}, width=${askB.width}`
      );
    }
    if (listenVis) {
      console.log(
        `[Layout Debug] Listen Window Bounds: height=${listenB.height}, width=${listenB.width}`
      );
    }
    ...
    if (askVis && listenVis) {
      if (strategy.primary === "left" || strategy.primary === "right") {
        // Side-by-side of header: stack vertically next to the header, centered on header
        const xBaseRight = headerBounds.x + headerBounds.width + PAD;
        const xBaseLeft = headerBounds.x - PAD; // subtract width per window below

        // Compute total height for vertical stack and center around header
        const totalH = askB.height + PAD + listenB.height;
        let yStart = headerBounds.y + headerBounds.height / 2 - totalH / 2;
        // Clamp within work area
        yStart = Math.max(
          workAreaY + PAD,
          Math.min(yStart, workAreaY + screenHeight - totalH - PAD)
        );

        // Order windows based on secondary: if below has more space, put ask on top then listen below; otherwise invert
        const firstIsAsk = strategy.secondary === "below";
        const firstB = firstIsAsk ? askB : listenB;
        const secondB = firstIsAsk ? listenB : askB;

        const firstName = firstIsAsk ? "ask" : "listen";
        const secondName = firstIsAsk ? "listen" : "ask";

        // X positions depending on side
        let xFor = (w) => xBaseRight; // default right
        if (strategy.primary === "left") {
          xFor = (w) => Math.round(xBaseLeft - w.width);
        }

        // Clamp X within work area
        const clampX = (x, w) => {
          const minX = workAreaX + PAD;
          const maxX = workAreaX + screenWidth - w.width - PAD;
          return Math.max(minX, Math.min(x, maxX));
        };

        const firstX = clampX(xFor(firstB), firstB);
        const firstY = Math.round(yStart);

        const secondX = clampX(xFor(secondB), secondB);
        const secondY = Math.round(yStart + firstB.height + PAD);

        layout[firstName] = {
          x: firstX,
          y: firstY,
          width: firstB.width,
          height: firstB.height,
        };
        layout[secondName] = {
          x: secondX,
          y: secondY,
          width: secondB.width,
          height: secondB.height,
        };
      } else {
        // Above/below of header: stack horizontally under/over header
      ...
      if (strategy.primary === "left" || strategy.primary === "right") {
        // Place single window next to header, vertically centered against header
        const centerYAbs = headerBounds.y + headerBounds.height / 2;
        let yAbs = Math.round(centerYAbs - winB.height / 2);
        // Clamp Y within work area
        yAbs = Math.max(
          workAreaY + PAD,
          Math.min(yAbs, workAreaY + screenHeight - winB.height - PAD)
        );

        let xAbs = headerBounds.x + headerBounds.width + PAD; // right
        if (strategy.primary === "left") {
          xAbs = headerBounds.x - PAD - winB.width;
        }
        // Clamp X within work area
        xAbs = Math.max(
          workAreaX + PAD,
          Math.min(xAbs, workAreaX + screenWidth - winB.width - PAD)
        );
  ...
  calculateShortcutSettingsWindowPosition() {
    const header = this.windowPool.get("header");
    // Align with app's window name
    const shortcutSettings = this.windowPool.get("shortcuts");
    ...
    let newX = Math.round(
      headerBounds.x + headerBounds.width / 2 - shortcutBounds.width / 2
    );
    let newY = Math.round(headerBounds.y);

    newX = Math.max(
      workArea.x,
      Math.min(newX, workArea.x + workArea.width - shortcutBounds.width)
    );
    newY = Math.max(
      workArea.y,
      Math.min(newY, workArea.y + workArea.height - shortcutBounds.height)
    );

    return {
      x: newX,
      y: newY,
      width: shortcutBounds.width,
      height: shortcutBounds.height,
    };
  }
  ...
  calculateNewPositionForDisplay(window, targetDisplayId) {
    ...
    const relativeX =
      (currentBounds.x - currentDisplay.workArea.x) /
      currentDisplay.workArea.width;
    const relativeY =
      (currentBounds.y - currentDisplay.workArea.y) /
      currentDisplay.workArea.height;

    const targetX =
      targetDisplay.workArea.x + targetDisplay.workArea.width * relativeX;
    const targetY =
      targetDisplay.workArea.y + targetDisplay.workArea.height * relativeY;
