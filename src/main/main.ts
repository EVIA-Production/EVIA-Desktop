...
// Optional: If you still want to hard block Windows during early dev, keep a guard here.
// For now we allow Windows to proceed so platform abstraction can function.
...
app.whenReady().then(() => {
  ...
  if (hw) platformApi.workspaceTweaks(hw);
  try {
    hw?.show();
  } catch {}
  try {
