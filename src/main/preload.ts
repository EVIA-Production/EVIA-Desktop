...
contextBridge.exposeInMainWorld("evia", {
  // Platform info bridged from main to renderer
  ...
  windows: {
    ...
    adjustAskHeight: (height: number) =>
      ipcRenderer.invoke("adjust-window-height", { winName: "ask", height }),
  ...
  permissions: {
    ...
    openSystemPreferences: (pane: string) =>
      ipcRenderer.invoke("permissions:open-system-preferences", pane),
