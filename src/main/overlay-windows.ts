...
// DEV/PROD LOADER NOTES
// In dev we run `npm run dev` which compiles main process TS to dist/main but does NOT copy HTML.
// Therefore `__dirname/../renderer/overlay.html` does NOT exist yet. We must either:
//  1) Load from the Vite dev server (preferred for HMR), OR
//  2) Load the source file directly from the workspace (app.getAppPath()/src/renderer/overlay.html)
// This helper centralizes that logic so missing dist assets don't break dev startup.

const DEV_SERVER_PORT = process.env.VITE_DEV_SERVER_PORT || "5174";
// Allow overriding full URL (e.g. when proxied) else construct default localhost URL
const DEV_SERVER_URL = (
  process.env.VITE_DEV_SERVER_URL || `http://localhost:${DEV_SERVER_PORT}`
).replace(/\/$/, "");

async function loadOverlayEntry(
  win: BrowserWindow,
  view: string,
  label: "header" | import("./overlay-windows").FeatureName
) {
  // If packaged, always load from compiled renderer assets (they must be copied there by build pipeline)
  const packaged = app.isPackaged;
  const wantsDev =
    !packaged &&
    (process.env.EVIA_DEV || process.env.NODE_ENV === "development");
  const packagedPath = path.join(__dirname, "../renderer/overlay.html");
  ...
  if (wantsDev) {
    // Try dev server first for fast iteration
    const devUrl = `${DEV_SERVER_URL}/overlay.html?view=${encodeURIComponent(
      view
    )}`;
    try {
      // Lightweight reachability check (HEAD preferred, but some dev servers may not implement it)
      const res = await fetch(devUrl, { method: "GET" });
      if (res.ok) {
        console.log(`[overlay-loader] DEV server reachable, loading ${devUrl}`);
        await win.loadURL(devUrl);
        return;
      } else {
        console.warn(
          `[overlay-loader] Dev server responded ${res.status}, falling back to source file`
        );
      }
    } catch (err) {
      console.warn(
        "[overlay-loader] Dev server not reachable, falling back to source file:",
        (err as Error).message
      );
    }
    // Fallback to raw source file (not copied to dist). Query params still passed for routing.
    console.log(
      `[overlay-loader] Loading source overlay.html directly from ${sourcePath}`
    );
    await win.loadFile(sourcePath, { query: { view } });
    return;
  }

  // Packaged or explicit prod: attempt packaged path. If missing, log helpful diagnostic.
  try {
    if (!fs.existsSync(packagedPath)) {
      console.error(
        "[overlay-loader] Packaged overlay.html missing at",
        packagedPath
      );
      console.error(
        "Ensure your build step copies renderer HTML into dist/renderer."
      );
    }
    await win.loadFile(packagedPath, { query: { view } });
  } catch (err) {
    console.error(
      "[overlay-loader] Failed to load packaged overlay:",
      (err as Error).message
    );
    // Last resort: try source path (useful when running unpackaged but NODE_ENV not set)
    if (!packaged && fs.existsSync(sourcePath)) {
      console.warn("[overlay-loader] Retrying with source overlay.html");
      await win.loadFile(sourcePath, { query: { view } });
    }
  }
}
...
function getOrCreateHeaderWindow(): BrowserWindow {
  ...
  // Load overlay (dev server → source file fallback → packaged file)
  // Fire and forget; promise chain logged by helper.
  loadOverlayEntry(headerWindow, "header", "header").catch((err) => {
    console.error("[overlay-loader] Failed to load header overlay:", err);
  });
...
function createChildWindow(name: FeatureName): BrowserWindow {
  ...
  // All windows load overlay entry (dev server aware)
  loadOverlayEntry(win, name, name).catch((err) => {
    console.error(`[overlay-loader] Failed to load overlay for ${name}:`, err);
  });
