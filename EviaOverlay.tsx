import React, { useState, useEffect } from 'react';
import EviaBar from './src/renderer/overlay/EviaBar';
import ListenView from './src/renderer/overlay/ListenView';
import AskView from './src/renderer/overlay/AskView';
import SettingsView from './src/renderer/overlay/SettingsView';

export const EviaOverlay: React.FC = () => {
    const [currentView, setCurrentView] = useState<'listen' | 'ask' | 'settings' | null>(null);
    const [isVisible, setIsVisible] = useState(true);
    const [isListening, setIsListening] = useState(false);
    const [language, setLanguage] = useState<'de' | 'en'>('de');

    // Global shortcuts handling
    useEffect(() => {
        const handleKeyDown = (e: KeyboardEvent) => {
            // ⌘ + \ = Toggle visibility
            if (e.metaKey && e.key === '\\') {
                e.preventDefault();
                setIsVisible(prev => !prev);
            }
            
            // ⌘ + Enter = Switch to Ask view
            if (e.metaKey && e.key === 'Enter') {
                e.preventDefault();
                setCurrentView('ask');
            }
        };

        document.addEventListener('keydown', handleKeyDown);
        return () => document.removeEventListener('keydown', handleKeyDown);
    }, []);

    // IPC communication with main process
    useEffect(() => {
        // Listen for IPC messages from main process
        if (window.electronAPI) {
            window.electronAPI.onToggleVisibility(() => {
                setIsVisible(prev => !prev);
            });

            window.electronAPI.onShowAskView(() => {
                setCurrentView('ask');
            });

            window.electronAPI.onShowListenView(() => {
                setCurrentView('listen');
            });
        }
    }, []);

    if (!isVisible) {
        return null;
    }

    return (
        <div className="evia-overlay">
            {/* EVIA Bar - Always visible */}
            <EviaBar
                currentView={currentView}
                onViewChange={setCurrentView}
                isListening={isListening}
                onListeningChange={setIsListening}
                language={language}
                onLanguageChange={setLanguage}
                onToggleVisibility={() => setIsVisible(false)}
            />

            {/* Glass Windows */}
            {currentView === 'listen' && (
                <div className="glass-window">
                    <ListenView
                        isListening={isListening}
                        language={language}
                        onClose={() => setCurrentView(null)}
                    />
                </div>
            )}

            {currentView === 'ask' && (
                <div className="glass-window">
                    <AskView
                        language={language}
                        onClose={() => setCurrentView(null)}
                    />
                </div>
            )}

            {currentView === 'settings' && (
                <div className="glass-window">
                    <SettingsView
                        language={language}
                        onLanguageChange={setLanguage}
                        onClose={() => setCurrentView(null)}
                    />
                </div>
            )}
        </div>
    );
};
